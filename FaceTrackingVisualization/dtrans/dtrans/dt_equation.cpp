...
*/
void DTEquation::_CalculateEquationSize(
    ...
    /* output param */ dt_size_type *n_row, dt_size_type *n_col)
{
    ...
    for ( ; i_tgt_triangle < target_mesh->m_n_triangle; i_tgt_triangle++)
    {
		n_corrs = tcdict->GetTriangleCorrsNumber(i_tgt_triangle);
        //n_corrs = __dt_GetTriangleCorrsNumber(tcdict, i_tgt_triangle);
        n_eqn += ((n_corrs > 0)? n_corrs: 1);
    }

    *n_row = 9 * n_eqn;
    *n_col = 3 * (target_mesh->m_n_vertex + target_mesh->m_n_triangle);
}

/* Allocate for coefficient matrix and rhs vector with proper size */
void DTEquation::_AllocDeformationEquation(
    const dtMeshModel *target_mesh, TriangleCorrsDict *tcdict,
    __dt_SparseMatrix *A_tri, __dt_DenseVector *C)
{
    dt_size_type n_row, n_col;
    _CalculateEquationSize(target_mesh, tcdict, &n_row, &n_col);
...
void DTEquation::_CalculateElementaryMatrix(
...
dt_index_type DTEquation::_GetVariableIndex(
    const dtMeshModel *model, dt_index_type i_triangle,
    dt_index_type i_vlocal, dt_index_type i_dim)
{
    const dtTriangle *triangle = model->m_triangle + i_triangle;
    ...
    if (i_vlocal < 3)  /* real vertex */
    {
        ...
        return 3*(model->m_n_vertex + i_triangle) + i_dim;
    }
}


dt_index_type DTEquation::_AppendElemMatrixToLinearSystem(
    const dtMeshModel *model, dt_index_type i_triangle,
    ...
    for (i_dim = 0; i_dim < 3; i_dim++)
    {
        for (i_eqn = 0; i_eqn < 3; i_eqn++, i_row++, j_row++)
        {
            ...
            for (i_vlocal = 0; i_vlocal < 4; i_vlocal++)
            {
                ...
                i_var = _GetVariableIndex(model, i_triangle, i_vlocal, i_dim);
...
*/
void DTEquation::_BuildCoefficientMatrix(
    ...
    __dt_SparseMatrix A)
{
    ...
    for ( ; i_triangle < target_ref->m_n_triangle; i_triangle++)
    {
        n_corrs = tcdict->GetTriangleCorrsNumber(i_triangle);
        ...
        if (n_corrs == 0)  /* i_triangle has no corresponded pieces */
        {
            ...
            _CalculateElementaryMatrix(sinvlist.inV[i_triangle], mat);
            i_row = _AppendElemMatrixToLinearSystem(
...
dt_index_type DTEquation::_AppendRhsVectorToLinearSystem(
    ...
    TriangleCorrsDict *tcdict, __dt_DenseVector C)
{
    ...
    for ( ; i_triangle < target_ref->m_n_triangle; i_triangle++)
    {
        ...
        if (n_corrs == 0)    /* i_triangle absent in tclist */
        {
            ...
            /* append I to C */
            i_row = _AppendRhsVectorToLinearSystem(T, C, i_row);
        }
        else 
        {
            for (i_entry = 0; i_entry < n_corrs; i_entry++)
            {
                ...
                i_src_triangle = tcdict->GetTriangleCorrsEntry(
                    i_triangle, i_entry)->i_src_triangle;
