...
{
    dtVector   ret;
    dtTriangle *surf = model->m_triangle + i_triangle;
    dtVertex   *v_l, *v_r = model->m_vertex + surf->i_vertex[0];

    dt_real_type u[3], v[3];  /* u = v1 - v0, v = v2 - v0 */

    v_l = model->m_vertex + surf->i_vertex[1];
    u[0] = v_l->x - v_r->x;
    u[1] = v_l->y - v_r->y;
    u[2] = v_l->z - v_r->z;

    v_l = model->m_vertex + surf->i_vertex[2];
    v[0] = v_l->x - v_r->x;
    v[1] = v_l->y - v_r->y;
    v[2] = v_l->z - v_r->z;
    ...
    const dtMeshModel *model, dt_index_type i_triangle, dtMatrix3x3 V)
{
    ...
    /* first column: v2 - v1 */
    v_l = model->m_vertex + surf->i_vertex[1];
    v_r = model->m_vertex + surf->i_vertex[0];
    ...
    const dtMeshModel *model, __dt_SurfaceInvVList *sinvlist)
{
    ...
    sinvlist->list_length = model->m_n_triangle;
    ...
    for ( ; i_surf < model->m_n_triangle; i_surf++)
    {
