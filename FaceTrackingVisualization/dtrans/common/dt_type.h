...
#define REALZERO 0.000001
#define ISREALEQUAL(x) (fabs(x) < REALZERO)
...
typedef struct __dtVertex_struct
{
	...
	bool isEqual(const __dtVertex_struct* v)
	{
		return (ISREALEQUAL(v->x-x) && ISREALEQUAL(v->y-y) && ISREALEQUAL(v->z-z));
	}
